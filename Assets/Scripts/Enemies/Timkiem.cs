...
public class Timkiem : DieukhienHanhvi
{
    [SerializeField]
    ...
    public override (float[] danger, float[] interest) GetSteering(float[] danger, float[] interest, AIData aiData)
    {
        ...
        {
            float result = Vector2.Dot(directionToTarget.normalized, PhuongHuong.Huong[i]);
    ...
    private void OnDrawGizmos()
    {
        ...
        if (Application.isPlaying && interestsTemp != null)
        {
            if (interestsTemp != null)
            {
                ...
                {
                    Gizmos.DrawRay(transform.position, PhuongHuong.Huong[i] * interestsTemp[i] * 2);
                }
