...
public class ShopSystem
{
    ...
    public ShopSystem(int size, int glod, float buyMarkUp, float sellMarkUp)
    {
        _availableGold = glod;
    ...
    {
        if (ConstainItem(data, out ShopSlot shopSlot))
        {
    ...
    public bool ConstainItem(InventoryItemData itemToAdd, out ShopSlot shopSlot)
    {
        shopSlot = _shopInventory.Find(i => i.ItemData == itemToAdd);
        return shopSlot != null;
    }

    public void PurschaseItem(InventoryItemData data, int amount)
    {
        if (!ConstainItem(data, out ShopSlot slot)) return;
